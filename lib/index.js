 ...
 * @type {String}
 */
//DEFAULT_TEMPLATE = '../templates/full/template-default.hbs',
DEFAULT_TEMPLATE = '../templates/full/template-default-colored.hbs',
//DEFAULT_TEMPLATE = '../templates/full/htmlreqres.hbs',
 ...
 * @param {Object} options.confluence - Mandatory set of options for confluence-api
 * @param {String} options.spaceId - Mandatory id of the Confluence space where to upload the report
 * @param {Object} collectionRunOptions - The set of all the collection run options.
 * @returns {*}
 */
PostmanHTMLFullReporter = function (newman, options, collectionRunOptions) {
    ...
    handlebars.registerHelper('sectionRow', function(options) {
        const firstColumnWidth = 30;
        let row = '{section}';
        const N = arguments.length-1;
        for(let i=0; i < N; i++) {
            row += `{column${i===0? `:width=${firstColumnWidth}%`: `:width:${(100-firstColumnWidth)/(N-1)}%`}}${arguments[i]}{column}`;
        }
        row += '{section}';
            ...
            assertions = _.transform(executions, function (result, currentExecution) {
                ...
                if (!_.has(traversedRequests, executionId)) {
                    ...
                    if (reducedExecution.response && _.isFunction(reducedExecution.response.toJSON)) {
                        ...
                        const responseStr = Buffer.from(stream).toString();
                        try {
...
/*
        ...
        var confluence = Confluence(options.confluence);
        confluence.getContentByPageTitle(options.spaceId, getTitle(this.summary), (err, res) => {
            if(err) {
                console.error(err);
                return;
            }
            ...
            if(res.code === 200) {
                confluence.putContent(options.spaceId, res.id, res.version, getTitle(this.summary), content, logRes);
            } else {
                confluence.postContent(options.spaceId, getTitle(this.summary), logRes);
            }
        });
*/
        this.exports.push({
            name: 'html-reporter-full',
            default: 'newman-run-report-full.html',
            path: options.export,
            content: compiler({
                timestamp: Date(),
                version: collectionRunOptions.newmanVersion,
...
module.exports = PostmanHTMLFullReporter;
